...
const SubtaskCard = memo(({ st, statusColorMap, openEditSubtask, onDelete, onDownloadFile, onPreviewFile, onDeleteFile, formatDate, canEdit = true, canDelete = true}) => {
    const statusExcluded = st.status_id === 4 || st.status_id === 6 || st.status_id === 7
    const overdue = st.dt_delivery_limit && dayjs(st.dt_delivery_limit).isBefore(dayjs(), 'day') && !statusExcluded
    const dueToday = st.dt_delivery_limit && dayjs(st.dt_delivery_limit).isSame(dayjs(), 'day') && !statusExcluded
    const dueSoon = st.dt_delivery_limit && !overdue && !dueToday && dayjs(st.dt_delivery_limit).diff(dayjs(), 'day') <= 7 && !statusExcluded
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-all hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
                ...
                {st.dt_delivery_limit && (
                    <span className={`flex items-center gap-1 ${overdue ? 'text-red-500 font-semibold' : dueToday ? 'text-amber-500 font-semibold' : dueSoon ? 'text-orange-500' : 'text-slate-400'}`}>
            ...
            {(st.files || []).length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                    {(st.files || []).map((f) => (
                        <div key={f.id} className="flex flex-col gap-0.5 text-xs bg-slate-100 dark:bg-slate-700 rounded-md px-2 py-1">
                            ...
                            {f.uploaded_by && (
                                <span className="text-[10px] text-slate-400 leading-tight">
                                    por {f.uploaded_by.name} · {formatDate(f.created_at)}
                                </span>
                            )}
                        </div>
                    ))}
            ...
            {st.created_by && (
                <div className="mt-3 text-[10px] text-slate-400 border-t border-slate-100 dark:border-slate-700 pt-2">
                    Creado por {st.created_by.name} · {formatDate(st.created_at)}
                </div>
            )}
        </div>
    )
