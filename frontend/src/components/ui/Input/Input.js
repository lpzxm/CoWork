...
const Input = React.forwardRef((props, ref) => {
    ...
    const measurePrefixGutter = useCallback((node) => {
        prefixNode.current = node

        if (isNil(node?.offsetWidth)) {
            setPrefixGutter(0)
            return
        }

        setPrefixGutter(node.offsetWidth)
    }, [])

    const measureSuffixGutter = useCallback((node) => {
        ...
        setSuffixGutter(node.offsetWidth)
    }, [])
