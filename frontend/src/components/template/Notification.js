...
export const Notification = ({ className }) => {
    ...
    const [notificationCountLoaded, setNotificationCountLoaded] = useState(false)
    ...
    const getNotificationCount = useCallback(async () => {
        ...
        setUnreadNotificationCount(resp.data.count)
        setNotificationCountLoaded(true)
    }, [])
    ...
    const markAllAsReadIcon = useMemo(
        () => <HiOutlineMailOpen className="text-xl" />,
        []
    )

    const noResult =
        notificationCountLoaded && !loading && !unreadNotification && notificationList.length === 0
